#!/bin/bash
# ──────────────────────────────────────────────────────────────────────────────
# GRPO Training: Qwen3.5-27B on a Single GPU via VERL + MegaTrain
# ──────────────────────────────────────────────────────────────────────────────
#
# Architecture:
#   SGLang (FP8)  ── rollout inference on GPU  (~27 GB weights + KV cache)
#   MegaTrain     ── training on CPU→GPU       (~9 GB transient GPU buffers)
...
#   CUDA_VISIBLE_DEVICES=0 bash examples/rl/run_qwen3_5_27b_megatrain.sh \
#       data.train_batch_size=4 actor_rollout_ref.rollout.n=4
# ──────────────────────────────────────────────────────────────────────────────
...
PROJECT_NAME=${PROJECT_NAME:-"GRPO-Qwen3_5-27B-MegaTrain"}
EXP_NAME=${EXP_NAME:-"grpo-27b-1gpu"}
...
python3 -m verl.trainer.main_ppo \
    ...
    "$@" 2>&1 | tee "${LOG_DIR}/grpo-qwen3_5-27b-${TIMESTAMP}.log"
